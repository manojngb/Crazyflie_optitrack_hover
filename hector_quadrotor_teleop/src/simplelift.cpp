...
public:
  Teleop()
  {
    ...
    if (control_mode_str == "twist")
    {
      ...
      velocity_publisher_ = node_handle_.advertise<geometry_msgs::Twist>("cmd_vel", 100);
      while (1)
      {
        velocity_.linear.x = 0;
        velocity_.linear.y = 0;
        velocity_.linear.z = 15000;
        velocity_.angular.z = 00;
